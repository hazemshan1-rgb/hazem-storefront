...
export function BenchmarkPreview() {
  ...
  const statusText = isAboveMedian
    ? t('benchmarkPreview.topHalf')
    : t('benchmarkPreview.belowBenchmarkNote', { pct: Math.round((fcr - 1.65) / 1.65 * 100) })
    ...
    <section ref={ref} className="scroll-reveal max-w-6xl mx-auto px-6 py-20">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 items-center">
        ...
        <div className="bg-[var(--color-surface)] border border-[var(--color-gold-muted)] rounded-sm p-8">
          <p className="text-[10px] tracking-widest uppercase text-[var(--color-gold)] mb-6">
            {t('benchmarkPreview.eyebrow')}
          </p>
          ...
          <div className="mb-6">
            <div className="flex justify-between items-baseline mb-3">
              ...
              <span className="text-xs font-semibold" style={{ color: colour }}>
                {isAboveMedian ? t('benchmarkPreview.aboveMedian') : t('benchmarkPreview.belowMedian')}
              </span>
            </div>
            ...
            <p className="text-[10px] text-[var(--color-text-muted)] mt-2">
              {t('benchmarkPreview.benchmarkNote', { fcr: fcr.toFixed(2) })} · {statusText}
            </p>
          </div>
