...
export function Navbar() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      scrolled
        ? 'bg-[var(--color-navy)]/90 backdrop-blur-md h-16 shadow-lg border-b border-[rgba(255,255,255,0.1)]'
        : 'bg-[var(--color-navy)] h-20 border-b border-[rgba(255,255,255,0.05)]'
    }`}>
        ...
        <nav className="hidden md:flex items-center gap-8">
          {links.map(l => (
            <NavLink
              ...
              className={({ isActive }) =>
                `text-xs tracking-widest uppercase transition-colors nav-link-premium ${
                  isActive
                    ? 'text-[var(--color-gold-cta)] active'
